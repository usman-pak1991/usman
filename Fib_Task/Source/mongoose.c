  ...
  if (realpath(opts->root_dir, t1) == NULL) {
    ...
    if (strlen(t2) < n1 || memcmp(t1, t2, n1) != 0) {
      ...
      if (is_index && fp == NULL) {
        ...
        printf("\n DBG2: SSI File Serving\n"); // Debug USMAN
        printf("SSI Pattern: %s\n",opts->ssi_pattern);
        printf("Root: %s\n",t1);
        printf("FullPath: %s\n",t2);
        mg_http_serve_ssi(c, t1, t2); // TODO: Modify this function to show variable values in client End.
        // currently it only process #include directrive
#endif
      } else {
  ...
  if (fp != NULL) {
    ...
    while ((ch = fgetc(fp)) != EOF) {
      if (intag && ch == '>' && buf[len - 1] == '-' && buf[len - 2] == '-') {
        ...
        } else if (sscanf(buf, "<!--#include sensor=\"%[^\"]", arg)) {
            printf("\nDBG3: SSI string parsing\n");
            printf("Buffer : %s\n",buf); // Debug USMAN
            printf("Arg : %s\n",arg);
            char temp[100];
            get_sensor_data(arg,temp);
            if(temp!=NULL)
            {
            	//char *temp = get_sensor_data(arg);
            	printf(temp);
            	mg_iobuf_append(&b, temp, strlen(temp), align);
            }
            else {
            	LOG(LL_ERROR, ("%s: sensor=%s invalid TAG", path, arg));
			}
